    ...
    <div class="col-md-9">
      ...
      <div class="col-md-6">
        ...
        <select class="<c:choose><c:when test="${pronounError == true}">is-invalid</c:when><c:when test="${pronounError == false}">is-valid</c:when><c:otherwise></c:otherwise></c:choose> form-select js-choice z-index-9 bg-transparent" aria-label=".form-select-sm" id="pronoun" name="pronoun">
          <option value="He/Him" <c:if test="${pronoun == 'He/Him'}">selected</c:if>>He/Him</option>
          <option value="She/Her" <c:if test="${pronoun == 'She/Her'}">selected</c:if>>She/Her</option>
          <option value="Other" <c:if test="${pronoun == 'Other'}">selected</c:if>>Other</option>
        </select>
